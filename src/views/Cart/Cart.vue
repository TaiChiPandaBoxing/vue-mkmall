                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        ...
                        <a class="input-add" @click="editCart('inc', item)">+</a>
                      </div>
                    </div>
                  </div>
                </div>
...
export default {
  ...
  methods: {
    init() {
      this.$http.get('/api/users/cartList').then((response) => {
    ...
    delCart() {
      this.$http.post('/api/users/cartDel', { productId: this.delItem.productId }).then((response) => {
        const res = response.data
        if (res.status === '1') {
          this.modalConfirm = false
          this.init()
          this.$store.commit('UPDATE_CART_COUNT', -this.delItem.productNum)
        }
      })
    },
    editCart(flag, goods) {
      ...
      this.$http.post('/api/users/cartEdit', {
        ...
        const res = response.data
        if (res.status === '1') {
          this.$store.commit('UPDATE_CART_COUNT', num)
        }
      })
    },
    toggleCheckAll() {

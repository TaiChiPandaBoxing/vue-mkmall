<template>
  <div class="address-wrap">
    ...
    <modal :md-show="modalConfirm2" @close="modalConfirm2 = false">
      <p slot="message">
        <span>地址列表至少需要一条数据，已无法继续删除.</span>
      </p>
      <div slot="btnGroup">
        <a href="javascript:;" class="btn btn-m" @click="modalConfirm2 = false">好的</a>
      </div>
    </modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    init() {
      this.$http.get('/api/users/addressList').then((response) => {
    ...
    setDefault(id) {
      this.$http.post('/api/users/setDefault', { addressId: id }).then((response) => {
        const res = response.data
        if (res.status === '1') {
          this.addressList = res.result
        }
